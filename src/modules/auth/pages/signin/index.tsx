...
export async function signInPageLoader() {
...
export async function signInPageAction({ request }: ActionFunctionArgs) {
  const formData = await request.formData();
  const username = formData.get('username');
  const password = formData.get('password');
  try {
    const response = await api.post<LoginResponse>(APIEndpoints.SIGN_IN, {
      email: username,
      password: password,
    });
